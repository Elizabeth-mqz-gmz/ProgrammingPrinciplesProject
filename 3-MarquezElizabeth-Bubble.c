...
#include <stdio.h>
int main() {
  int vec[10000], lenght=0, aux=0, num=0, i, j, l, fail, win=0;
  //DESCRIPCIÓN DEL PROGRAMA
  printf("Este programa recibe un conjunto de numeros enteros con signo y se encarga de ordenarlos de menor a mayor.\n");
  //RECEPCIÓN DE LOS DATOS DEL CONJUNTO POR EL USUARIO
  while (!win){
    fail = 0;
    printf("Ingrese la cantidad de numeros enteros que desea ordenar, el conjunto puede contener entre 1 y 10,000 elementos.\n");
    scanf("%d", &lenght);
    //VALIDAR QUE EL NÚMERO DE ELEMENTOS DEL CONJUNTO NO SEA MAYOR AL LÍMITE SUPERIOR
    if(lenght > 10000){
      printf("El numero de elementos es superior al limite establecido.\n Ingrese nuevamente el numero de elementos a ordenar.\n");
      fail = 1;
    }
    //VALIDAR QUE EL NÚMERO DE ELEMENTOS DEL CONJUNTO NO SEA MENOR AL LÍMITE INFERIOR
    if (lenght < 1) {
      printf("El numero de elementos es inferior al limite establecido.\n Ingrese nuevamente el numero de elementos a ordenar.\n");
      fail = 1;
    }
    win = (!fail) ? 1:0;
  }

  printf("Ingrese los elementos del conjunto a ordenar, recuerde que los numeros deben estar en un rango entre -20,000 y 20,000.\n");
  ...
  for (i = 0; i < lenght; i++) {
    ...
    if (num > 20000 || num < -20000) {
      printf("El numero no entra en el rango establecido entre -20,000 y 20,000. Ingrese un numero dentro del rango.\n");
      i-=1;
    }
    else
